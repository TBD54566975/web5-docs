...
<LanguageSwitcher languages="JavaScript, Kotlin" /># Issuing a Known Customer Credential

Known Customer Credentials (KCCs) are [Verifiable Credentials](/verifiable-credentials/what-are-vcs) designed to streamline the Know Your Customer (KYC) process for tbDEX protocol users. KCCs help in gaining access to PFIs providing regulated financial services.
...
The SOIPv2 Authorization request above that you as the issuer send to your customer contains the following key information about you as well as a [Presentation Definition](/verifiable-credentials/presentation-definition).
...
To convert the signed VC JWT into a `VerifiableCredential` object you can use the [`parseJwt()` method](/verifiable-credentials/jwt-to-vc#decoding-jwt). Here is what your decoded **Known Customer Credential** would look like:
