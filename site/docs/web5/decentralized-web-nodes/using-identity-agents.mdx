...
Identity Agents are a specialized type of [agent](https://developer.tbd.website/web5/decentralized-web-nodes/agents/) that act as personal identity managers.  Similar to how a password manager securely stores login credentials for different web apps, Identity Agents manage your [DIDs](https://developer.tbd.website/web5/decentralized-identifiers/what-are-dids).
...
To add [DWNs](https://developer.tbd.website/web5/decentralized-web-nodes/what-are-dwns/) as service endpoints to the DIDs that will be managed by the Identity Agent, you can add the endpoints as `didOptions`.
...
* The `kms` field stands for [Key Management Service](/web5/decentralized-identifiers/key-management) and can securely store and manage the cryptographic keys associated with a DID. If you don't specify a particular key management service, such as AWS Key Management Service, Web5 will default to an in-memory key manager.
:::
