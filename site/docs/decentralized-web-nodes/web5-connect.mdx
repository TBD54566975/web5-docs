...
The Web5 JS SDK provides a simple interface to get started building Web5 applications quickly. It provides APIs to use [Decentralized Identifiers (DIDs)](/web5/decentralized-identifiers/what-are-dids) and [Decentralized Web Nodes (DWNs)](/web5/decentralized-web-nodes/what-are-dwns) right away by initializing the SDK through `Web5.connect()`.
<br />
...
Using `Web5.connect()`, an instance of Web5 is created that enables an app to connect to an existing decentralized identifier (DID) either by direct creation or connection to an [identity agent app](/web5/decentralized-web-nodes/agents) or create a new one. The method also creates a locally running Decentralized Web Node (DWN) and associates it with the DID.
...
Enables an app to request connection to a user's local identity app (like a desktop or mobile [agent](/web5/decentralized-web-nodes/agents) - work is underway for reference apps of each), or generate an in-app DID to represent the user (e.g. if the user does not have an identity app)
