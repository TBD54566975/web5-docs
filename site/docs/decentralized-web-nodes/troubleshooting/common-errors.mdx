...
# Common Errors
If the suggested steps here don't work, please [open an issue](https://github.com/TBD54566975/developer.tbd.website/issues) or post your questions in the `#web5` channel on [Web5 Discord](https://discord.com/invite/TBD).
...
This error occurs when you attempt to sign a verifiable credential using the String version of a DID. To resolve this, sign with a [Portable DID](/glossary#portable-did) so that your private key is used to cryptographically sign the credential.

## Error: ProtocolAuthorizationProtocolNotFound: unable to find protocol definition

This error occurs when attempting to interact with a DWN that does not have the required protocol installed. To solve this, **ensure both parties have successfully [installed the protocol definition](/decentralized-web-nodes/what-are-protocols#installing-a-protocol)**.
