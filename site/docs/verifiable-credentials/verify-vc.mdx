...
Upon receipt, the presentation of the VCs is encoded as [JWTs](/verifiable-credentials/jwt-to-vc#what-is-a-jwt) and can be verified via the `VerifiableCredentials.verify()` function.
...
Since the VC JWTs are part of the larger [Presentation JSON object](/verifiable-credentials/presentation-exchange#create-presentation), they must be extracted and verified individually.
...
Upon receipt, a Verifiable Presentation is encoded as a [JWT](/verifiable-credentials/jwt-to-vc#what-is-a-jwt) and can be verified via the `VerifiablePresentation.verify()` function.
...
Once you're confident that the VCs are valid, you can proceed to [parse the JWTs into VC objects](/verifiable-credentials/jwt-to-vc) such that you may work with the credentials' data.
