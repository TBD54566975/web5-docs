...
The Web5 CLI is a Rust-based command-line tool for creating and managing [Decentralized Identifiers (DIDs)](/web5/decentralized-identifiers/what-are-dids) and [Verifiable Credentials (VCs)](/web5/verifiable-credentials/what-are-vcs). 

This guide covers CLI installation and usage for DID and VC management.

:::note
Alternatively, you can use the [Web5 SDK](/) to create [DIDs](/web5/decentralized-identifiers/how-to-create-did) and [VCs](/web5/verifiable-credentials/vc-issuance).
::: 
...
The `web5 did resolve` command resolves a DID to retrieve its [DID Document](/web5/decentralized-identifiers/did_documents)
...
## Create a VC
The `web5 vc create` command creates a VC. By default, this command creates an unsigned VC in JSON format. If a Portable DID is provided, this command outputs a signed VC in [JWT](/web5/verifiable-credentials/jwt-to-vc/#what-is-a-jwt) format.
