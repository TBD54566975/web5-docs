...
This tutorial demonstrates how to use Web5.js to create an app that stores users' book reviews within their [DWNs](/decentralized-web-nodes/what-are-dwns) so that they can be accessed by any app on the web that has [permission](/decentralized-web-nodes/what-are-dwns#authorization).
...
The first thing we want to do is **connect**. In Web5 apps, this replaces the traditional login step. Instead of centralized accounts that belong to one application, users have [DIDs](/decentralized-identifiers/what-are-dids) and [DWNs](/decentralized-web-nodes/what-are-dwns), decentralized identity and storage, respectively.
...
The objects that are written to a DWN are called records. To query for records from a user's DWN, we call `web5.dwn.records.query()` and pass in [filters](/decentralized-web-nodes/query-from-dwn#filterable-record-properties) to find the specific records we want. In our case, we want to query for all records that follow the Review schema.
...
* URI of the schema
* format of the data
* [publicity](/decentralized-web-nodes/publishing-records) of the data. Because we specified the records as published, they are available for public queries without requiring authorization.
